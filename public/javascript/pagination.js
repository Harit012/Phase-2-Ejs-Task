$(document).on("click", ".pagination-btn", function (e) {
  ...
  let currentPage = Number(nameOfBtn.slice(4, nameOfBtn.length));
  let operation = nameOfBtn.slice(0, 3);
  ...
  if (operation === "prv") {
    ...
    if (currentPage == -1) {
      ...
      $(`#prv-${currentPage + 1}`).attr({ id: `prv-${currentPage}` });
      $(`#nxt-${currentPage + 1}`).attr({ id: `nxt-${currentPage}` });
    }
  } else {
    currentPage = currentPage + 1;
    if (false) {
